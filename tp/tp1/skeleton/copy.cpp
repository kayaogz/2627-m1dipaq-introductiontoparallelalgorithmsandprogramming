...
//  Exercise 1: Copying an array using SIMD  --  SKELETON
//
...
void benchmark(int N, int REPS)
{
  ...
  float *A = NULL; // TODO: allocate N floats
  float *B = NULL; // TODO: allocate N floats
  // TODO: initialize A[i] = i

  double t0, t_scalar, t_simd;

  t0 = now();
  // TODO: call copy_scalar REPS times
  t_scalar = now() - t0;

  t0 = now();
  // TODO: call copy_simd REPS times
  t_simd = now() - t0;
...
int main()
{
  ...
  // -------------------------------------------------------------------
  // TODO: reset B[i] = 0 with a scalar loop before the test
  copy_scalar(A, B, N);
  check(A, B, N, "scalar");
  ...
  // TODO: reset B[i] = 0 with a scalar loop before the test
  // TODO: call copy_simd_any on the first M elements, then check the result
